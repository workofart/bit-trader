...
const calculateCorrelation = (priceArr1, priceArr2) => ss.sampleCorrelation(priceArr1, priceArr2);
...
exports.processCorrelation = (tickerBase, ticker, data, timestamp) => {
    ...
    if (Object.keys(priceArr).length === global.CORRELATION_PERIOD && numTickerRecords === TICKERS.length) {
        ...
        const basePriceArr = _.map(priceArr, item => parseFloat(item[tickerBase]));
        ...
        for (t of TICKERS) {
            ...
            if (t !== tickerBase) {
                ...
                const targetPriceArr = _.map(priceArr, item => parseFloat(item[t]));

                if (targetPriceArr.length === basePriceArr.length) {
                    const result = calculateCorrelation(basePriceArr, targetPriceArr);
                    ...
                    corrArr.push({ corr: result, tickerTarget: t, tickerBase });
                }
            }
        }
...
exports.calculateRSI = (close, subscore, ticker) => {
    if (close.length > global.RSI && flag.RSI) {
        return new Promise((resolve) => {
            tulind.indicators.rsi.indicator([close], [global.RSI], (err, results) => {
                const rsi = results[0];
...
exports.calculateDEMA_SMA_CROSS = (close, subscore) => {
    if (close.length > global.DEMA && flag.DEMA_SMA_CROSS) {
        let dema,
            sma;
        return new Promise((resolve) => {
            tulind.indicators.dema.indicator([close], [global.DEMA], (err, results) => {
                dema = results[0];

                tulind.indicators.sma.indicator([close], [global.SMA], (err, results) => {
                    sma = results[0];
...
exports.calculatePSAR = async (high, low, close, subscore) => {
    if (high.length > 1 && low.length > 1 && close.length > 1 && flag.PSAR) {
        try {
            const results = await tulind.indicators.psar.indicator([high, low], [global.PSAR_STEP, global.PSAR_MAX]);
            const psar = results[0];
...
exports.calculateADX = async (high, low, close, subscore) => {
    ...
    if (high.length > 1 && low.length > 1 && close.length > 1 && flag.ADX) {
        const inputData = [high, low, close];
        // util.log(JSON.stringify(inputData))
        try {
            const results = await tulind.indicators.adx.indicator(inputData, [global.ADX]);
            const adx = results[0];
            // Strong trend
...
exports.calculateBB_RSI = async (close, period = global.RSI) => {
    if (close.length > period && flag.RSI) {
        try {
            // console.time('BB_RSI');
            const rsi = await RSI_FUNC(close, [period]);
            const bb_score = await BB(close, period, global.BB_STD_DEV);
            // console.timeEnd('BB_RSI');
            const { bb_lower, bb_upper } = bb_score;
            // Long position
            if (rsi > 0 && rsi < global.LOWER_RSI && close[close.length - 1] <= bb_lower * (1 + 0.002)) {
                return {
                    indicatorValue: 10, rsi, bb_lower, bb_upper,
                };
            }
            // Short position
            ...
            return {
                indicatorValue: 0, rsi, bb_lower, bb_upper,
            };
        } catch (e) {
...
const BB = async (close, period, stdDev = global.BB_STD_DEV) => {
    try {
        const results = await tulind.indicators.bbands.indicator([close], [period, stdDev]);
...
const RSI_FUNC = async (close, period) => {
    try {
        const results = await tulind.indicators.rsi.indicator([close], [period]);
        const rsi = results[0];
...
const ADX_FUNC = async (high, low, close, period) => {
    try {
        const results = await tulind.indicators.adx.indicator([high, low, close], [period]);
        const adx = results[0];
