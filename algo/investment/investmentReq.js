...
class InvestmentReq {
    ...
    static repeatedSellReq(ticker, price) {
        const lastPrice = global.currencyWallet[ticker].bearSellPrice;
        return lastPrice === undefined || lastPrice === 0 || price > lastPrice;
    }

    static repeatedBuyReq(ticker, price) {
        const lastPrice = global.currencyWallet[ticker].repeatedBuyPrice;
        return lastPrice === undefined || lastPrice === 0 || price < lastPrice;
    }
    ...
    static stopLossReq(ticker, price) {
        const lastPrice = global.currencyWallet[ticker].price;
    ...
    static currencyBalanceReq(ticker) {
        const minNotional = _.find(minAmount, item => item.pair === ticker).minNotional;
    ...
    /**
	 * If an immediate loss occurs after a buy, this will lower the min-profit
	 * requirement
	 * @param ticker
	 * @returns {boolean|*}
	 */
    ...
    static maxProfitStopLimitReq(ticker) {
        const price = global.latestPrice[ticker];
